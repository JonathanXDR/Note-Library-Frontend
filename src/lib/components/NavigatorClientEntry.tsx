...
import { useSoftNavLifecycle } from '../hooks/use-soft-nav-lifecycle'
import { useTitleManager } from '../hooks/use-title-manager'
import { BaseProviders } from '../providers/BaseProviders'
import { NavigatorAppRegistration } from '../utils/navigator-app-registry'
...
export function NavigatorClientEntry({
  ...
  const [
    { location, error, routeStateMap, appPayload, navigateOnError, isLoading },
    { handleHistoryUpdate },
  ] = useNavigator({
    initialLocation,
    appName,
  ...
  return (
    <BaseProviders
      appName={appName}
      wasServerRendered={wasServerRendered}
      dataRouterEnabled={false}
    >
