...
async function fetchLazyData(url: string): Promise<LazyLoadItemDataAttributes> {
  ...
  return response.json() as Promise<LazyLoadItemDataAttributes>
}

const fetchDfdTasksData = async (): Promise<LazyDfdNewTasksData> => {
  const response = await fetch(
    '/in-product-messaging/dfd-new-tasks-indicator.json'
  )
  ...
  return response.json() as Promise<LazyDfdNewTasksData>
}
...
const UserStatusNavItem = memo(function UserStatusNavItem({
  lazyLoadItemData,
  onClick,
}: UserStatusItemProps) {
  return (
 ...
 */
function GlobalUserNavDrawerDialog(
  props: GlobalUserNavDrawerProps & { onClose: DialogProps['onClose'] }
) {
  const [lazyLoadItemData, setLazyLoadItemData] =
    useState<LazyLoadItemData | null>(null)
  const [lazyDfdNewTasksData, setLazyDfdNewTasksData] =
    useState<LazyDfdNewTasksData | null>(null)
  const [showUserStatusDialog, setShowUserStatusDialog] = useState(false)
  const [showFeaturePreviewDialog, setShowFeaturePreviewDialog] =
    useState(false)
  ...
  useEffect(() => {
    if (!lazyLoadItemData) {
      const fetchData = async () => {
        try {
          const lazyItemData = await fetchLazyData(
            props.lazyLoadItemDataFetchUrl
          )
  ...
  return error ? (
    ...
    >
      {showUserStatusDialog && (
        <UserStatusDialog onClose={onUserStatusClosed} />
      )}
      {showFeaturePreviewDialog && (
        <FeaturePreviewDialog
          onClose={() => setShowFeaturePreviewDialog(false)}
          login={props.owner.login}
        />
      )}
      <ActionList variant="full">
        <UserStatusNavItem
          lazyLoadItemData={lazyLoadItemData}
          onClick={openUserStatusDialog}
        />
        ...
        <NavLink
          href={`${profilePath}?tab=repositories`}
          icon={RepoIcon}
          analyticsAction="YOUR_REPOSITORIES"
        >
          Your repositories
        </NavLink>
        {props.showCopilot && (
        ...
        <NavLink
          ...
          icon={ProjectIcon}
          analyticsAction="YOUR_PROJECTS"
        >
          Your projects
        </NavLink>
        <NavLink
          href={`${profilePath}?tab=stars`}
          icon={StarIcon}
          analyticsAction="YOUR_STARS"
        >
          Your stars
        </NavLink>
        {props.showGists && (
        ...
        {props.showOrganizations && (
          <NavLink
            ...
            analyticsAction="YOUR_ORGANIZATIONS"
          >
            Your organizations
          </NavLink>
        )}
        ...
        {props.showEnterprise && (
          <NavLink
            href={props.yourEnterpriseUrl}
            icon={GlobeIcon}
            analyticsAction="YOUR_ENTERPRISE"
          >
            Your enterprise
          </NavLink>
        )}
        ...
        {props.showUpgrade && (
          <UpgradeNavItem lazyLoadItemData={lazyLoadItemData} />
        )}
        ...
        <NavLink
          ...
          icon={GearIcon}
          analyticsAction="SETTINGS"
        >
          Settings
        </NavLink>
        {props.showEnterpriseSettings && (
          <NavLink
            href={props.enterpriseSettingsUrl}
            icon={GlobeIcon}
            analyticsAction="ENTERPRISE_SETTINGS"
          >
            Enterprise settings
          </NavLink>
        )}
        ...
        <NavLink
          ...
          icon={BrowserIcon}
          analyticsAction="MARKETINGWEBSITE"
        >
          GitHub Website
        </NavLink>
