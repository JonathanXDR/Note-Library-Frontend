...
function isExpectedError(error: Error): boolean {
  ...
  if (
    EXPECTED_NETWORK_ERROR_TYPES.has(error.name) &&
    EXPECTED_NETWORK_ERROR_MESSAGES.has(error.message)
  )
    return true
  ...
  if (
    error.name.startsWith('ApiError') &&
    EXPECTED_NETWORK_ERROR_MESSAGES.has(error.message)
  )
    return true
...
async function report(context: PlatformReportBrowserErrorInput) {
  ...
  const url = document.head?.querySelector<HTMLMetaElement>(
...
function errorContext(
  error: PlatformJavascriptError,
  context: ErrorContext = {}
): PlatformReportBrowserErrorInput {
...
function isExtensionError(stack: PlatformStackframe[]): boolean {
  return stack.some(
    (frame) =>
      extensions.test(frame.filename) || extensions.test(frame.function)
  )
}

function pageRelease() {
  return document.head?.querySelector<HTMLMetaElement>('meta[name="release"]')
    ?.content
}
...
export function pageUser() {
  const login = document.head?.querySelector<HTMLMetaElement>(
