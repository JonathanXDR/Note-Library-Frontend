...
import { isResponseError, ResponseError } from '../types/response-error'
import { reportError } from '../utils/failbot'
...
export function handleIfNotReported<T extends object>(
  error: T,
  report: (err: T) => void
) {
...
function isNoRouteFoundError(
  routeError: unknown
): routeError is ErrorResponse & { status: 404 } {
...
function BaseRouteErrorBoundary({
  appName,
  routeError,
}: {
  appName: string
